...
def extract_product_info(single_contents, product_request):
    ...
    product_dict = {
        ...
        'Flinn_product_ids': product_id,
        'Flinn_product_names': product_name,
        'Flinn_product_quantities': product_quantity,
        'Flinn_product_prices': product_price,
        'Flinn_product_urls': product_url
    }
    return product_dict


def extract_sub_product_info(single_product):
        ...
        for single_contents in content_json:
            ...
            if not product_request.find('ul', class_='product-page__info--options list'):
                ...
                for single_product in sub_product:
                    product_dict = extract_sub_product_info(single_product)
                    save_product_data(product_dict)
...
def save_product_data(product_dict):
    if product_dict is not None:
        ...
        articles_df.drop_duplicates(subset=['Flinn_product_ids', 'Flinn_product_names'], keep='first', inplace=True)
        if os.path.isfile(f'{file_name}.csv'):
            articles_df.to_csv(f'{file_name}.csv', index=False, header=False, mode='a')
        else:
            articles_df.to_csv(f'{file_name}.csv', index=False)
        logging.info(f"Saved product: {product_dict.get('product_names', 'N/A')}")
